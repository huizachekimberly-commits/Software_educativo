"""
Optimize assets/videos/unit_2_activity_5.mp4 for web playback.

The original is 720p, ~12 Mbps, 15 MB for a 10-second looping clip.
This re-encodes it to H.264 (~1.5-2 Mbps, faststart, 540p max) so that
decoding is light and the loop does not stutter during option clicks.

The original file is backed up as unit_2_activity_5.original.mp4.
"""
import os
import shutil
import subprocess
...
SRC = os.path.join(VIDEO_DIR, "unit_2_activity_5.mp4")
BAK = os.path.join(VIDEO_DIR, "unit_2_activity_5.original.mp4")
DST = os.path.join(VIDEO_DIR, "unit_2_activity_5.optimized.mp4")
...
def main():
    ...
    # Probe dimensions with a quick ffprobe-equivalent using ffmpeg -i
    ...
    # no audio (the loop video is muted anyway), yuv420p for browser compat.
    cmd = [
        ffmpeg,
        "-y",
        "-hide_banner",
